# Demo-режим Neuro ROP Demo

Этот репозиторий — демо-версия Neuro ROP (НейроРОП демо), а не production-продукт. Документ описывает runtime-флаг `DEMO_MODE`: локальный стенд на замороженном snapshot. Не упрощает UI «Контроля сделок».
...
При `DEMO_MODE=true` frontend показывает `Сделка {deal_id}` вместо сохранённого `deal.title` и `Менеджер {manager_id}` вместо `manager_name`. Это только слой отображения:

* SQLite, snapshot и файлы отчётов не меняются;
* исходные `deal.title` и `manager_name` остаются в состоянии и уходят в живые LLM-вызовы;
* в FULL Markdown, рекомендациях и текстовых блоках UI перед показом подменяется только точное текущее значение `deal.title` и `manager_name`;
* российские телефоны в показанном тексте маскируются звёздочками, видны последние 4 цифры;
* в письмах и прочих показанных текстах `praktikm` / `ПрактикМ` заменяются звёздочками той же длины;
* в заголовке окна коммуникации живое ФИО не показывается: известный `manager_name` становится `Менеджер {id}`, иное имя заменяется названием канала;
* номер сделки остаётся на экране, но ссылка на карточку Bitrix не строится и не открывается;
* логины экрана «Команда» не маскируются;
* при `DEMO_MODE=false` названия остаются как в production, включая запасной вариант `Сделка #{id}`, ссылки на карточку снова работают, телефоны и бренд не маскируются.
